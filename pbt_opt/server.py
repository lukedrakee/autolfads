#!/usr/bin/env python3
"""
Population Based Training (PBT) Server for LFADS/RADICaL
...
import copy
import signal
import tf_compat  # TF2 compatibility module
...
def outlogger(*args):
    global outlog_fid
    if not tf_compat.file_exists(outlog_fid):
        with tf_compat.file_open(outlog_fid, 'w') as f:
            f.write("")
    with tf_compat.file_open(outlog_fid, 'a') as f:
...
def wlogger(wlog):
    global worklog_fid
    if not tf_compat.file_exists(worklog_fid):
        with tf_compat.file_open(worklog_fid, 'w') as f:
            f.write("")
    w_lines = "".join(wlog)
    with tf_compat.file_open(worklog_fid, 'a') as f:
        print('{}'.format(w_lines), file=f)


def hplogger(hplog):
    global hplog_fid
    if not tf_compat.file_exists(hplog_fid):
        with tf_compat.file_open(hplog_fid, 'w') as f:
            f.write("")
    hp_lines = "".join(hplog)
    with tf_compat.file_open(hplog_fid, 'a') as f:
...
class Process:
    ...
    def __init__(self, host_id, host_ip, start_cmd, db_name, host_zone, docker_name="pbt_container", server_ip="0.0.0.0"):
        self.process_uid = next(Process._id_counter)
    ...
    def get_status(self):
        ...
        # if it takes thrice than usual running time, mark the process as fail
...
class Worker:
    ...
    _id_counter = itertools.count()
    _id_counter2 = itertools.count()

    def __init__(self, worker_save_path, epochs_per_generation, worker_uid=None):
        if not self.init_existing_db():
            self.worker_uid = worker_uid if worker_uid is not None else next(Worker._id_counter)
    ...
    def init_existing_db(self):
        try:
            ...
            if self.epochs_per_generation is None or self.generation is None:
                return False
            else:
                return True
        except:
    ...
    def assign_job(self, job):
        ...
        if self.status == 'running':
            ...
            self.assigned_job_id = next(Worker._id_counter2)
            kind_flag = 1
            if 'kind' in hps:
...
class Workers:
    ...
    def assign_jobs(self, jobs):
        ...
        :param jobs: list of tuples in the form (hyperparameters, ckpt_ckpt_path)
        :return:
        """
    ...
    def process_workers(self, computer):
        """
        assigns workers to the processes and wait until all the workers are assigned
        """
...
class HyperParams:
    """Collection of all the hyperparameters"""
    def __init__(self):
        self.n_hps = 0
    ...
    def sample_hps(self, mode, num_samples=1):
        ...
        hp_samps = [v[1].sample(mode, num_samples) for v in list(self.hp_dict.items())]
        # format the output as [{hp1: val11, hp2:val12, ..}, {hp1:val21, hp2:val22,...}]
        hp_names = [list(n.keys())[0] for n in hp_samps]  # get the hp names
        values_mat = [list(q.values())[0] for q in hp_samps]  # prepare the value matrix
        ...
        values_mat = list(map(list, zip(*values_mat)))
...
class HyperParam:
    ...
    def sample(self, mode, num_samples):
        ...
        if isinstance(mode, list):
            ...
            if isinstance(self.value, tuple):  # value is range
                ...
                samp = [v.item() for v in samp.astype(self.dtype)]
            else:
    ...
    def fixvalue(self, value):
        ...
        value = np.array(value).astype(self.dtype).item()
        return value
...
class Population:
    ...
    def __init__(self, computer, hyperparams,
        ...
        self.num_no_best_to_stop = num_no_best_to_stop  # stop the training if the change in the mean performance is less than this
    ...
    def run_generation_ps(self):
        # run workers on the processes
        self.workers.process_workers(self.computer)
        # wait until all the workers become ready
        cir_char = cycle(['-', '\\', '|', '/'])
            ...
            if hp_obj.explorable:
                if hp_obj.explore_method == 'perturb':
                    ...
                    if isinstance(hp_obj.explore_param, dict):
                        ...
                        samp_prob = 1.0
                        # Python 3: use isinstance(x, str) instead of basestring
                        samp_mode = hp_obj.explore_param if isinstance(hp_obj.explore_param, str) else \
                            (hp_obj.init_sample_mode if isinstance(hp_obj.init_sample_mode, str) else 'rand')
    ...
    def train(self):
        ...
        try:
            ...
            while not self.end_training():
                ...
                with tf_compat.file_open(os.path.join(self.server_log_path, 'best_worker.sofar'), 'wb') as f:
...
class Server:
    def __init__(self, name, run_save_path, num_workers=40, epochs_per_generation=100,
        ...
        DB.db['process'].delete_many({})  # Updated from .remove() for modern pymongo
        global outlog_fid
        global worklog_fid
        global hplog_fid
        if not tf_compat.file_exists(server_log_path):
            tf_compat.makedirs(server_log_path)
        ...
        if force_overwrite:
            DB.db['worker'].delete_many({})  # Updated from .remove() for modern pymongo
            with tf_compat.file_open(outlog_fid, 'w') as f:
                f.write("")
            with tf_compat.file_open(worklog_fid, 'w') as f:
                f.write("")
    ...
    def start_pbt(self):
        if not self.force_overwrite and tf_compat.file_exists(os.path.join(self.server_log_path, 'best_worker.done')):
            with tf_compat.file_open(os.path.join(self.server_log_path, 'best_worker.done'), 'r') as f:
        ...
        for p, v in self.__dict__.items():
            # Python 3: use type checks instead of types.InstanceType
            if not hasattr(v, '__dict__') or isinstance(v, (str, int, float, bool, list, dict, tuple)):
                param_dict[p] = v

        outlogger(param_dict)

        with tf_compat.file_open(os.path.join(self.server_log_path, 'pbt_params.json'), 'w') as jsonfile:
        ...
        os.system('setterm -cursor on')
        if best_worker:
            with tf_compat.file_open(os.path.join(self.server_log_path, 'best_worker.done'), 'wb') as f:
